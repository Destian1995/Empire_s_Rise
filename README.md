...
1. **Структура проекта**
   - Основные файлы проекта:
     ...
     - `game_process.py` - цикл игры который обрабатывает действия игрока и переключает действия ИИ между собой. А так же включает в себя интерфейс с кнопками который запускает отдельные файлы py.
